...
const Navbar = () => {
  ...
  const isActive = (path) => {
    ...
    if (path === `/profile/${currentUser?.username}`) return location.pathname === path;
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border z-10 md:relative md:border-t-0 md:border-r md:h-screen md:w-32">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                to={effectivePath}
                className={`nav-item ${isActive(item.path) && !isDisabled ? 'active' : ''} ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            ...
            <div className="p-4 bg-card rounded-lg">
              <Link to={`/profile/${currentUser.username}`} className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">@{currentUser.username || 'username'}</p>
                </div>
              </Link>
            </div>
