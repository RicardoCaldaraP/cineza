...
const ReviewCard = ({
  ...
  useEffect(() => {
    const fetchAuthorAndMovie = async () => {
      ...
      const { data: authorData, error: authorError } = await supabase
        ...
        .single();
      if (authorError)
        console.error("Error fetching review author:", authorError);
      else setAuthor(authorData);

      const { data: movieData, error: movieError } = await supabase
        .from("movies")
        .select(
          "id, tmdb_id, media_type, title, year, poster_url, genre, backdrop_url"
        )
  ...
  const movieLink =
    movie.tmdb_id && movie.media_type
      ? `/item/${movie.media_type}/${movie.tmdb_id}`
      : `/movie/${movie.id}`;
  ...
  return (
    ...
    >
      ...
      <Link
        to={movieLink}
        className="block my-3 p-3 bg-secondary/50 rounded-lg hover:bg-secondary transition-colors"
      >
