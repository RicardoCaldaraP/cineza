...
const TMDB_API_KEY =
  "84a758e0a08ceyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI4NGE3NThlMGEwOGMyYjVkNDFmMDM4ZjZhNDRmNTkzYyIsIm5iZiI6MTc0NjQ2NDAxMC40OCwic3ViIjoiNjgxOGVkMGE0ODZhZDY4ZmViNjQwMTYzIiwic2NvcGVzIjpbImFwaV9yZWFkIl0sInZlcnNpb24iOjF9.UbpCJM8JyZdiAWF7NcQLuVSoTkknoPdJ1V8HA9HuQwk2b5d41f038f6a44f593c";
...
const fetchFromTMDB = async (endpoint, params = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message || `TMDB API Error: ${response.status}`
      );
    }
...
const getGenreNamesByIds = (genreIds, mediaType) => {
  ...
  if (Object.keys(map).length === 0) {
    console.warn(
      `Genre map for ${mediaType} not loaded yet. Returning raw IDs.`
    );
...
const mapTMDBToCinezaMovie = (tmdbItem, mediaTypeParam) => {
  const isMovie = mediaTypeParam
    ? mediaTypeParam === "movie"
    : tmdbItem.title !== undefined || tmdbItem.media_type === "movie";
  ...
  return {
    ...
    poster_url: tmdbItem.poster_path
      ? `${TMDB_IMAGE_BASE_URL}${tmdbItem.poster_path}`
      : null,
    backdrop_url: tmdbItem.backdrop_path
      ? `${TMDB_BACKDROP_IMAGE_BASE_URL}${tmdbItem.backdrop_path}`
      : null,
    release_date: isMovie ? tmdbItem.release_date : tmdbItem.first_air_date,
    year:
      parseInt(
        (isMovie ? tmdbItem.release_date : tmdbItem.first_air_date)?.split(
          "-"
        )[0] || 0,
        10
      ) || null,
...
export const getGenresTMDB = async (mediaType = "movie") => {
  const relevantPromise =
    mediaType === "movie" ? genresPromiseMovie : genresPromiseTV;
  const relevantMap = mediaType === "movie" ? genreMapMovie : genreMapTV;

  if (Object.keys(relevantMap).length > 0) {
    return Object.values(relevantMap).map((name) => ({
      id: parseInt(
        Object.keys(relevantMap).find((key) => relevantMap[key] === name)
      ),
      name,
    }));
  }
  ...
  const promise = fetchFromTMDB(`genre/${mediaType}/list`)
    .then((data) => {
      const newMap = {};
    ...
    .catch((error) => {
      ...
      if (mediaType === "movie") genresPromiseMovie = null;
      else genresPromiseTV = null;
      return [];
    });
...
export const searchTMDB = async (
  ...
  await Promise.all(
    [genresPromiseMovie, genresPromiseTV].filter((p) => p !== null)
  );
  ...
  try {
    ...
    console.error(
      `Error in searchTMDB for query "${query}", mediaType "${mediaType}":`,
      error
    );
...
export const getTMDBMovieDetails = async (tmdbId, mediaType = "movie") => {
  ...
  try {
    ...
    cinezaMovie.genre =
      (data.genres || []).map((g) => g.name).join(", ") || "Desconhecido";
    ...
    if (mediaType === "movie" && data.credits?.crew) {
      ...
      cinezaMovie.creators = data.created_by
    ...
    console.error(
      `Error in getTMDBMovieDetails for ID ${tmdbId}, mediaType ${mediaType}:`,
      error
    );
    return null;
  }
...
export const getPopularTMDB = async (
  mediaType = "movie",
  page = 1,
  genreIds = []
) => {
  ...
  try {
    ...
    return {
      results: (data.results || [])
        ...
        .map((item) =>
          mapTMDBToCinezaMovie({ ...item, media_type: mediaType })
        ),
      totalPages: data.total_pages || 0,
    };
  } catch (error) {
...
export const getTrendingTMDB = async (
  ...
  try {
    ...
    console.error(
      `Error in getTrendingTMDB for mediaType ${mediaType}:`,
      error
    );
    return [];
  }
...
export const ensureMovieInDb = async (
  tmdbMovieData,
  retries = 3,
  baseDelay = 100
) => {
  if (!tmdbMovieData || !tmdbMovieData.tmdb_id) {
    console.error(
      "ensureMovieInDb: tmdb_id é obrigatório. Dados recebidos:",
      tmdbMovieData
    );
    throw new Error(
      "Dados do filme TMDB inválidos para salvar no banco: tmdb_id ausente."
    );
  }
  if (!tmdbMovieData.media_type) {
    console.error(
      "ensureMovieInDb: media_type é obrigatório. Dados recebidos:",
      tmdbMovieData
    );
    throw new Error(
      "Dados do filme TMDB inválidos para salvar no banco: media_type ausente."
    );
  }
  ...
  try {
    ...
    if (fetchError) {
      console.error(
        `Erro ao buscar filme no DB (tmdb_id: ${tmdbMovieData.tmdb_id}, media_type: ${tmdbMovieData.media_type}):`,
        fetchError.message
      );
    ...
    const genreString = getGenreNamesByIds(
      tmdbMovieData.genre_ids,
      tmdbMovieData.media_type
    );
    ...
    if (insertError) {
      if (insertError.code === "23505") {
        console.warn(
          `ensureMovieInDb: Conflito de chave duplicada (23505) ao inserir tmdb_id: ${tmdbMovieData.tmdb_id}, media_type: ${tmdbMovieData.media_type}. Tentando buscar novamente.`
        );
        for (let i = 0; i < retries; i++) {
          await delay(baseDelay * Math.pow(2, i));
          let { data: concurrentlyInsertedMovie, error: refetchError } =
            await supabase
              .from("movies")
              .select("*")
              .eq("tmdb_id", tmdbMovieData.tmdb_id)
          ...
          if (refetchError) {
            console.error(
              `Erro na tentativa ${
                i + 1
              } de re-buscar filme após erro de duplicata (tmdb_id: ${
            ...
            if (i === retries - 1)
              throw new Error(
                `Erro ao re-buscar filme após duplicata (múltiplas tentativas): ${refetchError.message}`
              );
          } else if (concurrentlyInsertedMovie) {
            console.log(
              `Filme tmdb_id: ${
                tmdbMovieData.tmdb_id
              } encontrado na tentativa ${i + 1} após duplicata.`
            );
            return concurrentlyInsertedMovie;
          }
          console.warn(
            `Tentativa ${i + 1} de re-buscar tmdb_id: ${
              tmdbMovieData.tmdb_id
            } falhou. Próxima tentativa em ${
              baseDelay * Math.pow(2, i + 1)
            }ms...`
          );
        }
        throw new Error(
          `Filme não encontrado após tentativa de inserção duplicada (tmdb_id: ${tmdbMovieData.tmdb_id}, media_type: ${tmdbMovieData.media_type}) e ${retries} tentativas de re-busca.`
        );
      }
      console.error(
        `Erro ao inserir filme no DB (tmdb_id: ${tmdbMovieData.tmdb_id}, media_type: ${tmdbMovieData.media_type}):`,
        insertError.message
      );
    ...
    if (!newMovie) {
      throw new Error(
        `Falha ao inserir novo filme no banco (tmdb_id: ${tmdbMovieData.tmdb_id}), nenhum dado retornado.`
      );
    }
    return newMovie;
  } catch (error) {
    console.error(
      `Exceção em ensureMovieInDb para tmdb_id ${tmdbMovieData.tmdb_id}:`,
      error.message
    );
    throw error;
  }
